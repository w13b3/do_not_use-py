...
class RsaTest(unittest.TestCase):
    ...
    def test_encryption_decryption(self):
        ...
        """
        alice_shared_key = get_shared_key(self.alices_private_key, self.bobs_public_key)
        alice_derived_key = get_derived_key(alice_shared_key)

        bob_shared_key = get_shared_key(self.bobs_private_key, self.alices_public_key)
        bob_derived_key = get_derived_key(bob_shared_key)

        # bob sends an encrypted message with alice's public key to alice
        message_to_alice = b'hello alice'
        encrypted_by_bob = encrypt_with_derived_key(message_to_alice, bob_derived_key)
        # alice receives the message and decrypt the message with her private key
        decrypted_by_alice = decrypt_with_derived_key(encrypted_by_bob, alice_derived_key)
        self.assertEqual(message_to_alice, decrypted_by_alice)

        # alice sends an encrypted message with bob's public key to bob
        message_to_bob = b'hello bob'
        encrypted_by_alice = encrypt_with_derived_key(message_to_bob, alice_derived_key)
        # bob receives the message and decrypt the message with his private key
        decrypted_by_bob = decrypt_with_derived_key(encrypted_by_alice, bob_derived_key)
